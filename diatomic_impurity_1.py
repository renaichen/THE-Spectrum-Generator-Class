...
#
        ...
        # ssp, scp, damper[tstep] = just_get_n8(ssp, scp, -f2old, dt2)
        # ssp, scp, damper[tstep] = rk_4th_n8(ssp, scp, -f2old, dt2)
# #-----------EOM integrator: using the velocity verlet algorithm-----------------------
        ...
        f2old = -1e2*np.exp(-0.5*(x2[tstep+1]-x1[tstep+1])) #+ np.exp(-0.5*(x1[tstep+1]-0.0))
